...
func validateFeatures(log logger.Log, target Target, engines []Engine) (compat.JSFeature, compat.CSSFeature, map[css_ast.D]compat.CSSPrefix, string) {
	...
	for _, engine := range engines {
		...
		// XTODO
			[]logger.MsgData{logger.MsgData{Text: text}})
	}
...
func validateSupported(log logger.Log, supported map[string]bool) (
	...
	for k, v := range supported {
		if js, ok := compat.StringToJSFeature[k]; ok {
			...
			if css, ok := compat.StringToCSSFeature[k]; ok {
				cssMask |= css
				if !v {
					cssFeature |= css
				}
...
func (ctx *internalContext) rebuild() rebuildState {
	...
	if build0 := ctx.activeBuild; build0 != nil {
		ctx.mutex.Unlock()
		build0.waitGroup.Wait()
		return build0.state
	}
...
func (ctx *internalContext) Rebuild2() BuildResult {
...
func (ctx *internalContext) Dispose() {
	...
	for _, f := range ctx.args.onDisposeCallbacks {
		go f()
	}
}
...
func rebuildImpl(args rebuildArgs, oldHashes map[string]string) (rebuildState, map[string]string) {
			...
			if args.write {
				...
				if args.options.WriteToStdout {
					...
					for _, result2 := range results {
						go func(result2 graph.OutputFile) {
							defer waitGroup.Done()
							fs.BeforeFileOpen()
							defer fs.AfterFileClose()
							if oldHash, ok := oldHashes[result2.AbsPath]; ok && oldHash == newHashes[result2.AbsPath] {
								if contents, err := ioutil.ReadFile(result2.AbsPath); err == nil && bytes.Equal(contents, result2.Contents) {
									// Skip writing out files that haven't changed since last time
									return
								}
							}
							if err := fs.MkdirAll(realFS, realFS.Dir(result2.AbsPath), 0755); err != nil {
								log.AddError(nil, logger.Range{}, fmt.Sprintf(
									"Failed to create output directory: %s", err.Error()))
							} else {
								var mode os.FileMode = 0666
								if result2.IsExecutable {
									mode = 0777
								}
								if err := ioutil.WriteFile(result2.AbsPath, result2.Contents, mode); err != nil {
									log.AddError(nil, logger.Range{}, fmt.Sprintf(
	...
	for _, onEnd := range args.onEndCallbacks {
		fromPlugin, thrown := onEnd.myfn(&result)
...
func loadPlugins(initialOptions *BuildOptions, fs fs.FS, log logger.Log, caches *cache.CacheSet) (
	...
	clone := make([]Plugin,0) // XTODO // append(make([]Plugin, 0, len(initialOptions.Plugins)), initialOptions.Plugins...)
	...
	for i, item := range clone {
		...
		onEnd := func(myfn func(*BuildResult) (OnEndResult, error)) {
		...
		onDispose := func(myfn func()) {
			onDisposeCallbacks = append(onDisposeCallbacks, myfn)
		}
		...
		if outputs := getObjectPropertyObject(result, "outputs"); outputs != nil {
			...
			for _, entry := range entries {
				...
				for j, child := range entry.entries {
					...
					second2 := prettyPrintByteCount(child.size)
					third2 := fmt.Sprintf("%.1f%%", percent)

					table = append(table, tableEntry{
						first:     first,
						firstLen:  utf8.RuneCountInString(first),
						second:    second2,
						secondLen: len(second2),
						third:     third2,
						thirdLen:  len(third2),
					})
